...
export const register = (name, email, password) => async (dispatch) => {
  try {
    ...
    if (error.response.status === 300) {
      console.log(email);
      dispatch({ type: USER_REGISTER_SUCCESS, payload: { isRegister: true, email: email } });
    }
    dispatch({
...
export const updateUserProfile = (user) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await SuperServer.put(`/api/users/profile`, user, config);
...
export const updateUserProfilePhoto = (user) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await SuperServer.post(`/api/users/profile/photo`, user, config);
    dispatch({ type: USER_UPDATE_PROFILE_SUCCESS, payload: data });
    dispatch({ type: USER_LOGIN_SUCCESS, payload: data });
